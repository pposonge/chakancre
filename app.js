...
window.renderHistoryTimeline = function(){
    ...
    let h='';
    processed.forEach(l=>{
        h+=`<tr class="hover:bg-slate-50"><td class="py-3 px-2 text-[11px] text-slate-500 font-mono whitespace-nowrap">${formatDisplayDate(l.date)}</td><td class="py-3 px-2 font-bold text-slate-800">${l.geckoName}</td><td class="py-3 px-2 font-semibold text-brand-600">${l.weight?l.weight+'g':'-'}</td><td class="py-3 px-2 text-[11px] text-emerald-600">${l.feed||'-'}</td><td class="py-3 px-2 text-[11px] text-blue-600">${l.clean||'-'}</td><td class="py-3 px-2"><button onclick="window.deleteLog('${l.date}',${l.geckoId})" class="text-slate-300 hover:text-red-500"><i class="fa-solid fa-xmark"></i></button></td></tr>`;
    });
