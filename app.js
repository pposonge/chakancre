...
// 스크롤 위치 완벽 보정 (어정쩡하게 올라가는 문제 해결)
window.scrollMainTo = function(el) {
    if (!el) return;
    const headerOffset = window.innerWidth >= 1024 ? 110 : 180;
            ...
            html+=`<div class="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden hover:shadow-md transition cursor-pointer group" onclick="window.openDetailModal(${g.id})">
                ...
                <div class="p-4 flex flex-col flex-1">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span class="text-[10px] text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded font-semibold">${age}</span>
                    </div>
                    <div class="space-y-1.5 text-[11px]">
                        ...
                        <div class="flex justify-between text-slate-500"><span>최근 관리</span> <span>${cd}</span></div>
                    </div>
                    ${linInfo ? `<div class="mt-3 pt-2 border-t border-slate-100 text-[9px] text-purple-600 text-center font-semibold">${linInfo}</div>` : ''}
                    ${breedInfo ? `<div class="mt-2 text-center w-full">${breedInfo}</div>` : ''}
                    <div class="mt-auto pt-4 flex justify-end">
                        <button type="button" onclick="event.stopPropagation();window.deleteGecko(${g.id})" class="text-slate-300 hover:text-red-500 transition text-xs flex items-center gap-1"><i class="fa-solid fa-trash-can"></i> 삭제</button>
                    </div>
                </div>
            </div>`;
        }else{
...
window.renderBreedingTable = function() {
    ...
    if (eggViewMode === 'list') {
        ...
        // 격자(에그트레이) 뷰: 알 모양 디자인 적용, 화면 잘림 방지
