...
let editingEggId=null;let editingGeckoId=null;let pendingImages=[];let currentDetailGeckoId=null;
let viewMode='grid'; // 개체관리 기본값 격자
...
function getLastCareSummary(id){
    const l=window.appData.logs.filter(x=>x.geckoId===id).sort((a,b)=>getSafeDate(b.date)-getSafeDate(a.date));
    if(!l.length)return {date:null,summary:'',html:''};
    const r=l[0];let p=[];
    if(r.feed)p.push(r.feed);
    if(r.clean)p.push(r.clean);
    let h='';
    if(r.feed||r.clean){
        h=`<span class="text-emerald-500">${r.feed||''}</span>${r.feed&&r.clean?'<span class="text-slate-300 mx-1">|</span>':''}<span class="text-blue-500">${r.clean||''}</span>`;
    }else{
        h=`<span class="text-slate-400">무게만</span>`;
    }
...
// 스크롤 위치 완벽 보정: header 높이 동적 계산으로 모바일/PC 모두 완벽한 최상단 이동 지원
...
window.renderMatrixTable=function(){
    ...
    paginatedGeckos.forEach(g=>{
        ...
        displayDates.forEach(d=>{
            ...
            h+=l?`<td class="py-3 px-2 text-xs min-w-[80px]"><div class="font-bold text-slate-700">${l.weight?l.weight+'g':'-'} ${diffHtml}</div><div class="text-[10px] mt-0.5"><span class="text-emerald-500">${l.feed||''}</span>${(l.feed&&l.clean)?'<span class="text-slate-300 mx-0.5">|</span>':''}<span class="text-blue-500">${l.clean||''}</span></div></td>`:`<td class="py-3 px-2 text-[10px] text-slate-300 min-w-[80px]">-</td>`;
        });
        h+=`</tr>`;
    });
    h+=`</tbody>`;
...
window.setGeckoView=function(mode){viewMode=mode;const g=document.getElementById('view-grid-btn'),l=document.getElementById('view-list-btn'),c=document.getElementById('gecko-container');if(mode==='grid'){g.classList.replace('bg-white','bg-slate-100');g.classList.replace('text-slate-400','text-brand-600');l.classList.replace('bg-slate-100','bg-white');l.classList.replace('text-brand-600','text-slate-400');c.className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 transition-all';}else{l.classList.replace('bg-white','bg-slate-100');l.classList.replace('text-slate-400','text-brand-600');g.classList.replace('bg-slate-100','bg-white');g.classList.replace('text-brand-600','text-slate-400');c.className='flex flex-col gap-3 transition-all';}window.renderGeckos();};
...
// 개체관리 카드 UI 개선: 허전함 제거 및 최근기록 추가
window.renderGeckos=function(wf){
    ...
    paginatedData.forEach(g=>{
        const w=getLastWeight(g.id),ws=w!==null?w+'g':'기록없음';
        const care=getLastCareSummary(g.id),cd=care.date?formatShortDate(care.date):'-';
        const age=getAge(g.hatchDate)||'나이모름';
        ...
        const linInfo=(father||mother)?`<i class="fa-solid fa-sitemap mr-1 text-purple-400"></i>${father?'♂'+father.name:''}${(father&&mother)?' × ':''}${mother?'♀'+mother.name:''}`:'';
        const childrenCount=window.appData.lineage.filter(l=>l.fatherId==g.id||l.motherId==g.id).length;
        
        let breedInfo='';
        if(childrenCount>0)breedInfo+=`<span class="px-2 py-0.5 bg-brand-50 text-brand-700 text-[10px] font-bold rounded">자녀 ${childrenCount}마리</span>`;
        else if(w!==null){
            if(w>=25)breedInfo+=`<span class="px-2 py-0.5 bg-slate-100 text-emerald-600 text-[10px] font-bold rounded">성체</span>`;
            else if(w>=15)breedInfo+=`<span class="px-2 py-0.5 bg-slate-100 text-blue-500 text-[10px] font-bold rounded">준성체</span>`;
            else if(w>=5)breedInfo+=`<span class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold rounded">아성체</span>`;
            else breedInfo+=`<span class="px-2 py-0.5 bg-slate-100 text-slate-400 text-[10px] font-bold rounded">베이비</span>`;
        }
        ...
        if(viewMode==='grid'){
            html+=`<div class="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden hover:shadow-md transition cursor-pointer group" onclick="window.openDetailModal(${g.id})">
                <div class="h-32 w-full bg-slate-50 relative border-b border-slate-100">
                    ${thHtml}
                    <span class="absolute top-2 right-2 px-2 py-0.5 bg-white/90 backdrop-blur ${gc} text-[10px] font-bold rounded-md shadow-sm">${gi} ${g.gender}</span>
                </div>
                <div class="p-4 flex flex-col flex-1">
                    <div class="flex justify-between items-start mb-3">
                        <h3 class="text-base font-bold text-slate-800 group-hover:text-brand-600 transition">${g.name}</h3>
                        <span class="text-[10px] text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded font-semibold shrink-0 ml-2">${age}</span>
                    </div>
                    <div class="space-y-1.5 text-[11px] mb-3">
                        <div class="flex justify-between text-slate-500"><span>모프</span> <span class="text-slate-700 font-semibold">${g.morph||'-'}</span></div>
                        <div class="flex justify-between text-slate-500"><span>최근 무게</span> <span class="text-brand-600 font-bold">${ws}</span></div>
                        <div class="flex justify-between text-slate-500">
                            <span>최근 관리</span> 
                            <div class="text-right">
                                <span>${cd}</span>
                                ${care.html ? `<div class="mt-0.5">${care.html}</div>` : ''}
                            </div>
                        </div>
                    </div>
                    ${linInfo ? `<div class="mt-auto pt-2 border-t border-slate-100 text-[9px] text-purple-600 text-center font-semibold mb-2">${linInfo}</div>` : ''}
                    <div class="${linInfo ? '' : 'mt-auto pt-3 border-t border-slate-100'} flex justify-between items-center w-full">
                        <div>${breedInfo}</div>
                        <button type="button" onclick="event.stopPropagation();window.deleteGecko(${g.id})" class="text-slate-300 hover:text-red-500 transition text-xs flex items-center gap-1"><i class="fa-solid fa-trash-can"></i></button>
                    </div>
                </div>
            </div>`;
        }else{
            html+=`<div class="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex items-center gap-4 hover:shadow-md transition cursor-pointer group" onclick="window.openDetailModal(${g.id})">
                ...
                <div class="flex-1 grid grid-cols-2 md:grid-cols-5 gap-2 items-center">
                    <div>
                        <div class="flex items-center gap-1.5"><h3 class="text-sm font-bold text-slate-800 group-hover:text-brand-600">${g.name}</h3><span class="px-1 py-0.5 bg-slate-100 ${gc} text-[9px] font-bold rounded">${gi}</span></div>
                        <p class="text-[10px] text-slate-400 mt-0.5">${g.morph||'-'}</p>
                        ${linInfo ? `<div class="text-[9px] text-purple-500 mt-0.5">${linInfo}</div>` : ''}
                    </div>
                    <div class="text-xs text-slate-500"><span class="text-[10px] text-slate-400 block mb-0.5">해칭일 (나이)</span>${g.hatchDate?formatDisplayDate(g.hatchDate):'-'} <span class="text-[9px] text-brand-600 font-semibold">(${age})</span></div>
                    <div class="text-xs">${breedInfo}</div>
                    <div class="text-xs font-bold text-brand-600"><span class="text-[10px] text-slate-400 block font-normal mb-0.5">최근 무게</span>${ws}</div>
                    <div class="text-xs text-slate-500"><span class="text-[10px] text-slate-400 block mb-0.5">최근 관리</span>${cd}${care.html?' <div class="text-[10px] font-bold mt-0.5">'+care.html+'</div>':''}</div>
                </div>
                <div class="shrink-0 flex gap-3 px-2">
                    <button type="button" onclick="event.stopPropagation();window.openGeckoModal(${g.id})" class="text-slate-400 hover:text-blue-500 transition"><i class="fa-solid fa-pen"></i></button>
                    <button type="button" onclick="event.stopPropagation();window.deleteGecko(${g.id})" class="text-slate-400 hover:text-red-500 transition"><i class="fa-solid fa-trash-can"></i></button>
                </div>
            </div>`;
        }
    });
...
window.openDetailModal=function(id){
    ...
    document.getElementById('detail-gender').innerText=g.gender;
    document.getElementById('detail-morph').innerText=g.morph||'모프 미상';
    document.getElementById('detail-hatch').innerText=formatDisplayDate(g.hatchDate)||'-';
    document.getElementById('detail-weight').innerText=w!==null?w+'g':'기록 없음';
    document.getElementById('detail-last-care').innerText=care.date?formatDisplayDate(care.date):'-';
    
    const careEl=document.getElementById('detail-care-status');
    if(careEl)careEl.innerHTML=care.html||'<span class="text-slate-300">-</span>';
    ...
    if(lin){
        ...
        if(f)info+=`아버지 <span class="text-blue-600">${f.name}</span> `;
        if(m)info+=`어머니 <span class="text-pink-600">${m.name}</span>`;
        if(children.length)info+=`<br><i class="fa-solid fa-baby text-brand-500 mr-1"></i><strong>자녀:</strong> ${children.map(c=>c.name).join(', ')}`;
        linEl.innerHTML=info;
        linEl.classList.remove('hidden');
    }else{
        const children=window.appData.lineage.filter(l=>l.fatherId==id||l.motherId==id).map(l=>window.appData.geckos.find(x=>x.id==l.childId)).filter(Boolean);
        if(children.length){
            linEl.innerHTML=`<i class="fa-solid fa-baby text-brand-500 mr-1"></i><strong>자녀:</strong> ${children.map(c=>c.name).join(', ')}`;
            linEl.classList.remove('hidden');
    ...
    if(g.images&&g.images.length){
        gallery.innerHTML=g.images.map(img=>`<img src="${img}" alt="gecko">`).join('');
...
window.populateDropdowns=function(){let mH='<option value="">수컷 선택</option>',fH='<option value="">암컷 선택</option>',fA='<option value="">암컷 선택</option>';window.appData.geckos.forEach(g=>{if(g.gender==='수')mH+=`<option value="${g.id}">${g.name}</option>`;if(g.gender==='암'){fH+=`<option value="${g.id}">${g.name}</option>`;fA+=`<option value="${g.id}">${g.name}</option>`;}});document.getElementById('breed-male').innerHTML=mH;document.getElementById('breed-female').innerHTML=fH;document.getElementById('infertile-female').innerHTML=fA;let lH='<option value="all">모든 개체보기</option>';window.appData.geckos.forEach(g=>lH+=`<option value="${g.id}">${g.name}</option>`);document.getElementById('filter-gecko').innerHTML=lH;};
...
window.renderLogInputForm=function(){
    ...
    const totalPages=Math.ceil(totalItems/LOCAL_ITEMS_PER_PAGE)||1;
    
    if(window.logFormPage>totalPages)window.logFormPage=totalPages;
    const paginatedData=sorted.slice((window.logFormPage-1)*LOCAL_ITEMS_PER_PAGE,window.logFormPage*LOCAL_ITEMS_PER_PAGE);
...
window.renderHistoryTimeline=function(){
    ...
    paginatedData.forEach(l=>{
        ...
        h+=`<tr class="hover:bg-slate-50/80">
            <td class="px-2 py-2 text-center truncate bg-white sticky left-0 z-10 shadow-[2px_0_4px_-1px_rgba(0,0,0,0.1)] lg:shadow-none lg:border-r lg:border-slate-100">
            ...
            <td class="px-1 py-2 text-center">
                <div class="flex flex-col items-center gap-0.5">
                    ${f!=='-'?`<span class="text-[10px] bg-emerald-50 text-emerald-600 px-1.5 rounded border border-emerald-100 whitespace-nowrap">${f}</span>`:''}
                    ${c!=='-'?`<span class="text-[10px] bg-blue-50 text-blue-500 px-1.5 rounded border border-blue-100 whitespace-nowrap">${c}</span>`:''}
                    ${f==='-'&&c==='-'?'<span class="text-slate-300 text-[10px]">-</span>':''}
                </div>
            </td>
            <td class="px-1 py-2 text-center">
    ...
    document.getElementById('history-timeline-body').innerHTML=h||`<tr><td colspan="4" class="text-center py-6 text-slate-400 text-xs">기록 없음</td></tr>`;
...
window.autoFillMateDate=function(){
    ...
    const mateDateInput=document.getElementById('mateDate');
    mateDateInput.value='';
    if(mateDateInput._flatpickr)mateDateInput._flatpickr.clear();
    
    if(maleId&&femaleId){
        const male=window.appData.geckos.find(g=>g.id==maleId);
        const female=window.appData.geckos.find(g=>g.id==femaleId);
        if(male&&female){
            const past=window.appData.eggs.filter(e=>e.male===male.name&&e.female===female.name&&e.mateDate);
            if(past.length>0){
...
window.saveBreeding=function(e){
    e.preventDefault();
    const bt=document.getElementById('breed-type').value;
    if(bt==='infertile'){
        const femaleId=document.getElementById('infertile-female').value;
        const femaleObj=window.appData.geckos.find(g=>g.id==femaleId);
        if(!femaleObj)return alert('암컷을 선택하세요.');
        const d={id:editingEggId||Date.now(),female:femaleObj.name,layDate:normalizeDateStr(document.getElementById('layDate').value),eggCount:parseInt(document.getElementById('eggCount').value)||0,memo:document.getElementById('eggMemo').value,type:'infertile'};
        if(!d.layDate)return alert('산란일을 입력하세요.');
        if(editingEggId){const i=window.appData.infertileEggs.findIndex(x=>x.id==editingEggId);if(i>-1)window.appData.infertileEggs[i]=d;}else window.appData.infertileEggs.push(d);
        saveData();window.resetBreedingForm(true);window.showToast("무정란 기록 저장");
        return;
    }
    
    const maleId=document.getElementById('breed-male').value;
    const femaleId=document.getElementById('breed-female').value;
    const maleObj=window.appData.geckos.find(g=>g.id==maleId);
    const femaleObj=window.appData.geckos.find(g=>g.id==femaleId);
    
    if(!maleObj||!femaleObj)return alert('수컷/암컷을 선택하세요.');
    
    const ed={male:maleObj.name,female:femaleObj.name,mateDate:normalizeDateStr(document.getElementById('mateDate').value),layDate:normalizeDateStr(document.getElementById('layDate').value),eggCount:parseInt(document.getElementById('eggCount').value)||0,targetTemp:document.getElementById('targetTemp').value,memo:document.getElementById('eggMemo').value};
    if(!ed.mateDate)return alert('교배일을 입력하세요.');
...
window.resetBreedingForm=function(preserveType=false){const currentType=document.getElementById('breed-type').value;editingEggId=null;document.getElementById('incubatorForm').reset();document.getElementById('eggCount').value=2;document.getElementById('btn-cancel-breed').classList.add('hidden');document.getElementById('btn-submit-breed').innerText='등록하기';if(!preserveType)window.setBreedType('fertile');else window.setBreedType(currentType);};

window.editEgg=function(id){
    const egg=window.appData.eggs.find(e=>e.id==id);
    if(!egg)return;
    window.setBreedType('fertile');
    editingEggId=id;
    const maleObj=window.appData.geckos.find(g=>g.name===egg.male);
    const femaleObj=window.appData.geckos.find(g=>g.name===egg.female);
    if(maleObj) document.getElementById('breed-male').value=maleObj.id;
    if(femaleObj) document.getElementById('breed-female').value=femaleObj.id;
    
    document.getElementById('mateDate').value=egg.mateDate;
    document.getElementById('layDate').value=egg.layDate||'';
    document.getElementById('eggCount').value=egg.eggCount;
    document.getElementById('targetTemp').value=egg.targetTemp;
    document.getElementById('eggMemo').value=egg.memo||'';
    document.getElementById('btn-cancel-breed').classList.remove('hidden');
    document.getElementById('btn-submit-breed').innerText='수정완료';
    window.scrollMainTo(document.getElementById('incubatorForm').closest('.bg-white'));
    setTimeout(initCalendars,50);
};

window.editInfertileEgg=function(id){
    const egg=window.appData.infertileEggs.find(e=>e.id==id);
    if(!egg)return;
    window.setBreedType('infertile');
    editingEggId=id;
    const femaleObj=window.appData.geckos.find(g=>g.name===egg.female);
    if(femaleObj) document.getElementById('infertile-female').value=femaleObj.id;
...
let eggViewMode = 'grid'; // 부화현황 모니터 기본값을 격자모드로 수정
window.setEggView = function(mode) {
    eggViewMode = mode;
    const gl = document.getElementById('egg-view-list-btn');
    const gg = document.getElementById('egg-view-grid-btn');
    if (mode === 'grid') {
        gg.classList.replace('bg-white', 'bg-slate-100');
        gg.classList.replace('text-slate-400', 'text-brand-600');
        gl.classList.replace('bg-slate-100', 'bg-white');
        gl.classList.replace('text-brand-600', 'text-slate-400');
    } else {
        gl.classList.replace('bg-white', 'bg-slate-100');
        gl.classList.replace('text-slate-400', 'text-brand-600');
        gg.classList.replace('bg-slate-100', 'bg-white');
        gg.classList.replace('text-brand-600', 'text-slate-400');
    }
    window.renderBreedingTable();
};
...
window.renderBreedingTable = function() {
    ...
    let container = document.getElementById('egg-view-container');
    if (!container) return;
    
    let h = '';
    if (eggViewMode === 'list') {
        h += `<div class="overflow-x-auto hide-scroll">
            <table class="w-full text-left text-xs border-collapse min-w-[650px] table-fixed">
                <colgroup>
                    <col class="w-[130px] md:w-[160px]"><col class="w-[95px] md:w-[110px]"><col class="w-[80px]"><col class="w-[110px]"><col class="w-[70px]"><col class="w-[60px]">
                </colgroup>
                <thead>
                    <tr class="bg-slate-50/50 text-[11px] font-semibold text-slate-500 border-b border-slate-200">
                        <th class="px-2 py-3 bg-slate-100 sticky left-0 z-20 shadow-[2px_0_4px_-1px_rgba(0,0,0,0.1)] lg:shadow-none lg:border-r lg:border-slate-200 text-center">매칭</th>
                        <th class="px-2 py-3 text-center">교배일<br><span class="text-[11px] font-bold text-amber-600">산란일</span></th>
                        <th class="px-2 py-3 text-center">알/온도</th>
                        <th class="px-3 py-3 text-center">해칭 예정</th>
                        <th class="px-3 py-3 text-center">D-Day</th>
                        <th class="px-2 py-3 text-center">작업</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">`;
        if (paginatedData.length === 0) h += `<tr><td colspan="6" class="text-center py-6 text-slate-400 text-xs">기록 없음</td></tr>`;
        else {
            paginatedData.forEach(egg => {
                ...
                h += `<tr class="hover:bg-slate-50/80 group">
                    <td class="px-2 py-3 font-semibold bg-white group-hover:bg-slate-50 sticky left-0 z-10 shadow-[2px_0_4px_-1px_rgba(0,0,0,0.1)] lg:shadow-none lg:border-r lg:border-slate-100 text-center cursor-pointer" onclick="window.openEggDetailModal(${egg.id})">
        ...
        h += `<div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-y-6 gap-x-2 p-4 md:p-6 bg-slate-50/50 rounded-xl">`;
        if (paginatedData.length === 0) h += `<div class="col-span-full py-6 text-center text-slate-400 text-xs">기록 없음</div>`;
        else {
            paginatedData.forEach(egg => {
                let eggBorder = 'border-brand-400';
                let eggText = 'text-brand-600';
                let eggBg = 'bg-white';
                let pulse = '';
                
                let displayDday = egg.diff.toString();
                
                if (egg.state === 'hatched') {
                    eggBorder = 'border-slate-300';
                    eggText = 'text-slate-400';
                    eggBg = 'bg-slate-50';
                    displayDday = '완료';
                } else if (egg.state === 'waiting') {
                    eggBorder = 'border-amber-300';
                    eggText = 'text-amber-500';
                    eggBg = 'bg-white';
                    displayDday = '대기';
                } else {
                    if (egg.diff === 0) {
                        displayDday = 'D-Day';
                        eggBorder = 'border-red-400';
                        eggText = 'text-red-500';
                        pulse = 'animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.3)]';
                    } else if (egg.diff > 0 && egg.diff <= 7) {
                        eggBorder = 'border-red-400';
                        eggText = 'text-red-500';
                        pulse = 'animate-pulse';
                    }
                ...
                h += `<div class="flex flex-col items-center justify-start cursor-pointer group hover:-translate-y-1 transition-transform" onclick="window.openEggDetailModal(${egg.id})">
                    <div class="egg-shape border-2 ${eggBorder} ${eggBg} flex flex-col items-center justify-center w-12 h-16 shadow-sm group-hover:shadow-md relative ${pulse}">
                        <span class="text-xs font-bold ${eggText}">${displayDday}</span>
                    </div>
                    <div class="mt-2 text-center w-full">
                        <span class="block text-[10px] font-bold text-slate-700 truncate w-full px-1">${egg.female}</span>
                        ${egg.layDate ? `<span class="block text-[9px] text-slate-400 font-mono mt-0.5">${formatShortDate(egg.layDate)}</span>` : ''}
                    </div>
                </div>`;
            });
        }
...
window.openEggDetailModal = function(id) {
    ...
    let eggs=[...window.appData.eggs];
    const femaleEggs = eggs.filter(e => e.female === egg.female).sort((a,b)=>getSafeDate(a.layDate||a.mateDate)-getSafeDate(b.layDate||b.mateDate));
    let clutch = '?';
    let clutchCount = 1;
    femaleEggs.forEach(e => {
        if(e.layDate) {
            if(e.id === egg.id) clutch = clutchCount;
            clutchCount++;
        }
    });
    ...
    document.getElementById('egg-detail-layDate').innerText = egg.layDate ? formatDisplayDate(egg.layDate) : '미산란';
    document.getElementById('egg-detail-exp').innerText = exp;
    document.getElementById('egg-detail-dday').innerText = dDay;
    document.getElementById('egg-detail-memo').innerText = egg.memo || '메모 없음';
    
    document.getElementById('eggDetailModal').classList.remove('hidden');
};

window.closeEggDetailModal = function() {
    currentDetailEggId = null;
    document.getElementById('eggDetailModal').classList.add('hidden');
};

window.editEggFromDetail = function() {
    if(currentDetailEggId) {
        const id = currentDetailEggId;
        window.closeEggDetailModal();
        window.editEgg(id);
    }
};

window.deleteEggFromDetail = function() {
    if(currentDetailEggId) {
        const id = currentDetailEggId;
        window.closeEggDetailModal();
        window.deleteEgg(id);
